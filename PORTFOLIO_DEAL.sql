...
/*
SPMPLN-14534: Cоздание нового прототипа portfolio_deal (обновленная версия от 04.09.2023). 
Название:	Витрина 5 - Сделка
Описание:	Основной массив данных в представляет собой сделки юридических лиц которые построены на витрине dm_corp_models.portfolio_all_core , клиенты которых входят IRBF портфель
Заказчик:	Угрин Артём Михайлович (ДМКФР)
Автор:		Кашин Алексей Сергеевич (ДМКФР)
Аналитик:	Метелкин Андрей Михайович (ГБК), Аракчеева Анастасия Артуровна (ДРПД)
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.portfolio_deal_tab - техническая таблица с типом хранения данных SCD-2.
	2) dm_corp_models.portfolio_deal – итоговая пользовательская вью.
--------------------
SPMPLN-15347,15206,16168,15210,18596,17193,19787,19845,21009,20181: см. до 04.02.2025
--------------------
SPMPLN-22616: изменение источника расчета остатков с portfolio_all_core на portfolio_deal_rest для 5NT, CFT
--------------------
SPMPLN-22898: замена источников STAGE на прямую реплику из АС ВР
*/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_deal_tab PURGE
/
CREATE TABLE sbxc_006_053.portfolio_deal_tab
(
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.portfolio_deal
/
CREATE VIEW sbxc_006_053.portfolio_deal
(
...
AS SELECT
	...
	, t_changed_dttm
	, t_process_task_id			
FROM sbxc_006_053.portfolio_deal_tab
WHERE t_deleted_flg = 0
	AND	t_active_flg = 1
/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_deal_off_bal PURGE
/
/* Поиск внебелансовых счетов */
CREATE TABLE sbxc_006_053.portfolio_deal_off_bal AS
SELECT /* Внебаланс */
	  dp.report_dt
	, a.account_id
FROM dm_core.d_account_base AS a
JOIN sbxc_006_053.ref_params AS dp
WHERE 1=1
...
/
COMPUTE STATS sbxc_006_053.portfolio_deal_off_bal
/
/* Модельный коэфицент кредитной конверсии CCF (В рамках расчета модели) */
DROP TABLE IF EXISTS sbxc_006_053.portfolio_ccf_dict PURGE
/
CREATE TABLE sbxc_006_053.portfolio_ccf_dict AS
WITH ccf_map AS (
	SELECT
...
/
COMPUTE STATS sbxc_006_053.portfolio_ccf_dict
/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_guar purge
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.portfolio_guar as 
SELECT
	  pr.report_dt
	, i.abs_id
	...
	, g.source_system_id
FROM sbxc_006_053.ref_params AS pr
...
/
COMPUTE STATS sbxc_006_053.portfolio_guar
/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_akkr purge
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.portfolio_akkr as 
SELECT
	  pr.report_dt
	, i.abs_id
	, i.instrument_id 			AS deal_stage_id
...
/
COMPUTE STATS sbxc_006_053.portfolio_akkr
/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_deal_ccf PURGE
/
/* Коэфицент кредитной конверсии CCF */
CREATE TABLE sbxc_006_053.portfolio_deal_ccf AS
WITH ccf AS (
	SELECT
...
WHERE i.rn = 1
/
COMPUTE STATS sbxc_006_053.portfolio_deal_ccf
/
/* Качественные/Групповые атрибуты Проекта*/
DROP TABLE IF EXISTS sbxc_006_053.proj_attr PURGE
/
CREATE TABLE sbxc_006_053.proj_attr AS
WITH rating AS (
	SELECT
		  dp.report_dt
	...
	CROSS JOIN sbxc_006_053.ref_params AS dp
...
, asvr_indicators AS (
	SELECT	
		...
		, c.gs_2
		, c.gs_9
	FROM sbxc_006_053.rating_calc_indicators c
	JOIN sbxc_006_053.portfolio_client_asvr p 
		ON c.client_asvr_id = p.client_asvr_id 
		AND c.calculation_asvr_id = p.calculation_asvr_id
	JOIN sbxc_006_053.ref_params AS rp
...
/
COMPUTE STATS sbxc_006_053.proj_attr 
/
/* Проектное финансирование, связь сделки АБС с проектом АСВР */
DROP TABLE IF EXISTS sbxc_006_053.poject_fin_deal_lnk PURGE
/
CREATE TABLE sbxc_006_053.poject_fin_deal_lnk AS
	...
	SELECT
		...
		, CASE
			...
			WHEN ad.external_system = 'DIA.BEANS'
				THEN 209
		  END AS t_srs_id
	FROM sbxc_006_053.ref_params AS rp
...
, proj_fin_risk_owner as (
	SELECT 
		...
		, guar.obj_id
		, prj.client_core_id
	FROM sbxc_006_053.ref_params AS dt 
	...
	/* Поручитель */
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS prj /* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с фильтром по t_deleted_flg */
...
LEFT JOIN sbxc_006_053.proj_attr AS g_attr
...
/
COMPUTE STATS sbxc_006_053.poject_fin_deal_lnk
/
INSERT INTO sbxc_006_053.portfolio_deal_tab
(
	  report_dt
	, client_core_id
	, proj_asvr_id
...
WITH model_irbf_portf AS (
	...
	SELECT
		  dp.report_dt
		, b.irbf_nm
	FROM sbxc_006_053.ref_params AS dp 
...
, client as (
	SELECT
		...
		, CASE
		  ...
		  END AS model_flg
	FROM sbxc_006_053.ref_params AS dp 
	JOIN sbxc_006_053.portfolio_client_asvr AS c	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с фильтром по t_deleted_flg */
...
, ccf_cls AS (
	SELECT
		...
		, c.ccf
		, c.model_ccf 
	FROM sbxc_006_053.portfolio_ccf_dict AS c	
)
, pac AS (
		SELECT
		  d.report_dt 
	...
	FROM sbxc_006_053.ref_params AS dp  
	JOIN sbxc_006_053.PORTFOLIO_DEAL_REST AS d	/* На ПРОДе использовать data_dm_corp_models.portfolio_deal_rest_tab с фильтрами по t_deleted_flg, t_active_flg */
	...
	FROM sbxc_006_053.ref_params AS dp  
	JOIN sbxc_006_053.FACTORING_DEAL_HIST AS fd
...
, risk_bearer AS (
	...
	FROM sbxc_006_053.ref_params AS dp  
	JOIN dm_asvr_ultra_gpb_d.v_ext_beans_deal AS d	/* На ПРОДе использовать data_dm_corp_models.portfolio_all_core_tab с фильтрами по t_deleted_flg, t_active_flg */
...
, deal AS (
	...
	/* Признак внебаланса */
	LEFT JOIN sbxc_006_053.portfolio_deal_off_bal AS a
		ON a.report_dt = d.report_dt
		AND a.account_id = d.account_id
	/* Проектное финансирование */
	LEFT JOIN sbxc_006_053.poject_fin_deal_lnk AS prj
		ON prj.report_dt = d.report_dt
