...
---манцова
/*
SPMPLN-14534: Cоздание нового прототипа portfolio_client_asvr (обновленная версия от 04.09.2023). 
Название:	Витрина 2 - Клиент АС ВР
Описание:	Основной массив данных в представляет собой клиентов АС ВР, полученный массив данных дополняется информацией информацией (необходимой для расчета входного вектора) о отрасль, irbf портфель, рейтингах, финансовых показателях, качественных показателях, триггерах дефолта и т.д.
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.portfolio_client_asvr_tab - техническая таблица.
	2) dm_corp_models.portfolio_client_asvr – итоговая пользовательская вью.
-----------------
*/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_client_asvr_tab PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.portfolio_client_asvr_tab
(
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.portfolio_client_asvr
/
CREATE VIEW sbxc_006_053.portfolio_client_asvr
(
...
AS SELECT 
	...
	, t_changed_dttm
	, t_process_task_id
FROM sbxc_006_053.portfolio_client_asvr_tab
WHERE t_deleted_flg = 0
/
/* Рейтинги */
DROP TABLE IF EXISTS sbxc_006_053.portfolio_client_asvr_rating PURGE
/
CREATE TABLE sbxc_006_053.portfolio_client_asvr_rating AS
WITH pd_scale AS (
	SELECT 
		  rating
		, `mid` AS `value`
...
WHERE rat1.rnk = 1
/
COMPUTE STATS sbxc_006_053.portfolio_client_asvr_rating
/
/* Финансовые фичи на основе фин отчености */
DROP TABLE IF EXISTS sbxc_006_053.lgd_finrep_feature_input PURGE
/
CREATE TABLE sbxc_006_053.lgd_finrep_feature_input AS 
WITH rat as (
	SELECT
		  dp.report_dt
		, r.client_asvr_id
		, r.report_date
	FROM sbxc_006_053.portfolio_client_asvr_rating AS r 
	JOIN sbxc_006_053.ref_params AS dp
...
WHERE ff.rn = 1
/
compute stats sbxc_006_053.lgd_finrep_feature_input
/
/* Финансовая отчетность */
drop table if EXISTS sbxc_006_053.finrep_stat_input PURGE
/
create table sbxc_006_053.finrep_stat_input AS 
WITH rat as (
	SELECT
		  dp.report_dt
		, r.client_asvr_id
...
WHERE rn = 1
/
COMPUTE STATS sbxc_006_053.finrep_stat_input
/
/* Данные реестра дефолтов */
DROP TABLE IF EXISTS sbxc_006_053.lgd_reestr_defaults_input PURGE
/
CREATE TABLE sbxc_006_053.lgd_reestr_defaults_input AS 
WITH rd1 AS (
	SELECT
		  dp.report_dt
		, rdt.asvr_id
...
WHERE rd2.rn = 1
/
COMPUTE STATS sbxc_006_053.lgd_reestr_defaults_input
/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_client_asvr_cntr_lnk PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.portfolio_client_asvr_cntr_lnk AS
/* контрактники */
SELECT
	  dp.report_dt
...
GROUP BY 
	...
	, cl.asvr_id
/
COMPUTE STATS sbxc_006_053.portfolio_client_asvr_cntr_lnk
/
INSERT INTO sbxc_006_053.portfolio_client_asvr_tab
(
  report_dt
	, client_asvr_id
	, client_core_id
...
, reg_top_grp AS ( 
	...
	FROM sbxc_006_053.asvr_group_hierarchy AS grp	/* в джобе использовать data_dm_corp_models.asvr_group_hierarchy_tab с фильтром на t_deleted_flg = 0 */
...
LEFT JOIN sbxc_006_053.portfolio_client_asvr_rating AS rp
	ON	rp.report_dt = a.report_dt
	AND rp.client_asvr_id = a.client_asvr_id
	AND rp.rating_type = 'RATING_PIT'
/* Рейтинги ttc */
LEFT JOIN sbxc_006_053.portfolio_client_asvr_rating AS rt
...
LEFT JOIN reg_top_grp AS tp
	...
	AND tp.top_rn = 1 
	AND a.client_type_cd = 'GROUP_OF_COMPANIES'
LEFT JOIN sbxc_006_053.portfolio_client_asvr_cntr_lnk AS cl
...
/
COMPUTE STATS sbxc_006_053.portfolio_client_asvr_tab
/
/* технические проверки прототипа */
/* Дубли по PK */
SELECT report_dt,client_stage_id, COUNT(*)
FROM sbxc_006_053.portfolio_client_asvr AS c
...
/* NULL в PK */
SELECT *
FROM sbxc_006_053.portfolio_client_asvr
