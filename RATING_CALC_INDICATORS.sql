...
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.RATING_CALC_INDICATORS_TAB	- техническая таблица с SCD-2.
	2) dm_corp_models.RATING_CALC_INDICATORS			– итоговая пользовательская вью.
*/
DROP TABLE IF EXISTS sbxc_006_053.rating_calc_indicators_tab PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.rating_calc_indicators_tab
(	
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.rating_calc_indicators
/
CREATE VIEW sbxc_006_053.rating_calc_indicators
(
...
/* Технические поля */
	...
	, t_changed_dttm
	, t_process_task_id
FROM sbxc_006_053.rating_calc_indicators_tab
WHERE t_deleted_flg = 0
	AND	t_active_flg = 1
/
DROP TABLE IF EXISTS sbxc_006_053.rating_calc_indicators_full PURGE
/
CREATE TABLE sbxc_006_053.rating_calc_indicators_full STORED AS PARQUET AS
WITH asvr_rating AS (
	SELECT
...
FROM sbxc_006_053.indicators_asvr /* на ETL использовать data-слой с фильтром на t_deleted_flg = 0 и t_active_flg = 1*/
)
, sort_prior AS (
SELECT 
	  client_asvr_id
...
FROM sort_prior
/
COMPUTE STATS sbxc_006_053.rating_calc_indicators_full
/
DROP TABLE IF EXISTS sbxc_006_053.rating_calc_indicators_tr PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.rating_calc_indicators_tr STORED AS PARQUET AS
SELECT
	  ei.client_asvr_id
...
FROM sbxc_006_053.rating_calc_indicators_full AS ei
WHERE ei.rn = 1
GROUP BY ei.client_asvr_id, ei.calculation_asvr_id, ei.calculation_korpro_guid
/
COMPUTE STATS sbxc_006_053.rating_calc_indicators_tr
/
INSERT INTO sbxc_006_053.rating_calc_indicators_tab
(
	  client_asvr_id
    , calculation_asvr_id
    , calculation_korpro_guid
...
WITH wgr0 AS (
	...
	FROM sbxc_006_053.rating_calc_indicators_tr
)
, wgr AS (
	SELECT
		  client_asvr_id
...
FROM wgr
/
COMPUTE STATS sbxc_006_053.rating_calc_indicators_tab
/
