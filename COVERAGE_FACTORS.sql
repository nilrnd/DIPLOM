...
/*
SPMPLN-14534: Cоздание нового прототипа coverage_factors (обновленная версия от 04.09.2023). 
...
Аналитик:	Метелкин Андрей Михайлович (ГБК), Аракчеева Анастасия Артуровна (ДРПД)
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.coverage_factors_TAB - техническая таблица с типом хранения данных SCD-2.
	2) dm_corp_models.coverage_factors – итоговая пользовательская вью.
--------------------
SPMPLN-15347,15206,16168,18596,19048,19787,20181,22252: см. в истории Битбакет
--------------------
SPMPLN-23256: Добавление атрибутов по сумме залогов 1 и 2 КК для учета в доле покрытия
*/
DROP TABLE IF EXISTS sbxc_006_053.coverage_factors_tab PURGE
/
CREATE TABLE sbxc_006_053.coverage_factors_tab
(
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.coverage_factors
/
CREATE VIEW sbxc_006_053.coverage_factors
(
...
AS SELECT
	...
	, t_changed_dttm
	, t_process_task_id			
FROM sbxc_006_053.coverage_factors_tab
WHERE t_deleted_flg = 0
	AND	t_active_flg = 1
/
DROP TABLE IF EXISTS sbxc_006_053.lgd_portfolio_collateral_split PURGE
/
 ...
 */
CREATE TABLE sbxc_006_053.lgd_portfolio_collateral_split AS 
WITH lnk AS (
...
, lnkcol AS (
	...
	FROM lnk AS l
	JOIN sbxc_006_053.portfolio_collateral AS c	/* На ПРОДе использовать data_dm_corp_models.portfolio_collateral_tab с дополнительными фильтрамм по t_deleted_flg, t_active_flg */
...
, split AS (
	...
	JOIN sbxc_006_053.portfolio_deal AS d	/* На ПРОДе использовать data_dm_corp_models.portfolio_deal_tab с дополнительными фильтрамм по t_deleted_flg, t_active_flg */
...
/
COMPUTE STATS sbxc_006_053.lgd_portfolio_collateral_split
/
DROP TABLE IF EXISTS sbxc_006_053.lgd_portfolio_collateral_vg_gr PURGE
/
CREATE TABLE sbxc_006_053.lgd_portfolio_collateral_vg_gr AS 
WITH split_group AS (
	SELECT
		  s.report_dt      
	...
	FROM sbxc_006_053.ref_params AS dp 
	JOIN sbxc_006_053.lgd_portfolio_collateral_split AS s
		ON s.report_dt = dp.report_dt
	/* Группы контрагента */
	LEFT JOIN sbxc_006_053.portfolio_client_asvr_grp AS gc	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_grp_tab с дополнительным фильтром по t_deleted_flg */
		ON	gc.report_dt = s.report_dt 
		AND gc.client_core_id = s.client_core_id
	/* Группа залогодатель */
	LEFT JOIN sbxc_006_053.portfolio_client_asvr_grp AS gp	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_grp_tab с дополнительным фильтром по t_deleted_flg */
...
/
COMPUTE STATS sbxc_006_053.lgd_portfolio_collateral_vg_gr
/
INSERT INTO sbxc_006_053.coverage_factors_tab
(
	  report_dt
	, client_core_id
	, deal_core_id
...
with split1 AS (
	...
	JOIN sbxc_006_053.lgd_portfolio_collateral_split AS s
		ON s.report_dt = dp.report_dt
	LEFT JOIN sbxc_006_053.lgd_portfolio_collateral_vg_gr AS vg
		...
		AND vg.coll_core_id = s.coll_core_id
	     /* Залогодатель*/
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS cp	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
