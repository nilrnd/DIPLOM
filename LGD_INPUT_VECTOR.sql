...
Описание:	Основной массив данных представляет собой входную витрину подаваемую на вход модели (GPB-CRD-LGD-CE-3.0) расчета значений LGD (агрегат Витрины 6 - Факторы покрытия обогащённой информацией из Витрина 2 - Клиент АС ВР).
	1) data_dm_corp_models.lgd_input_vector_tab - техническая таблица с типом хранения данных SCD-2.
	2) dm_corp_models.lgd_input_vector – итоговая пользовательская вью.
--------------------
*/
DROP TABLE IF EXISTS sbxc_006_053.lgd_input_vector_tab PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.lgd_input_vector_tab
(
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.lgd_input_vector
/
CREATE VIEW sbxc_006_053.lgd_input_vector
(
...
AS SELECT
	...
	, t_changed_dttm
	, t_process_task_id			
FROM sbxc_006_053.lgd_input_vector_tab
WHERE t_deleted_flg = 0
	AND	t_active_flg = 1
/
DROP TABLE IF EXISTS sbxc_006_053.lgd_collateral_portfolio_unity PURGE 
/
CREATE TABLE sbxc_006_053.lgd_collateral_portfolio_unity AS
WITH cls_excl AS (
...
, pre_cf AS (
	...
	FROM sbxc_006_053.coverage_factors AS f	/* На ПРОДе использовать data_dm_corp_models.coverage_factors_tab с дополнительными фильтрамм по t_deleted_flg, t_active_flg */
	JOIN sbxc_006_053.ref_params AS dp
		ON	dp.report_dt = f.report_dt
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS p	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с фильтром по t_deleted_flg */
...
, cb_excl AS (
	...
	FROM sbxc_006_053.portfolio_deal AS pcd
	JOIN sbxc_006_053.ref_params AS rp
		ON	pcd.report_dt = rp.report_dt
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS c
...
/
COMPUTE STATS sbxc_006_053.lgd_collateral_portfolio_unity
/
INSERT INTO sbxc_006_053.lgd_input_vector_tab
(
	  report_dt
	, client_core_id
	, client_asvr_id
...
WITH unity_core AS (
	SELECT
		...
		, d.t_source_system_id 
	FROM sbxc_006_053.ref_params AS dp 
	JOIN sbxc_006_053.lgd_collateral_portfolio_unity AS d
		ON	d.report_dt  = dp.report_dt 
...
, unity_client AS (
	SELECT
		...
		, d.t_source_system_id
	FROM unity_core AS d
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS a	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
		ON	a.report_dt = d.report_dt 
		AND a.client_core_id = d.def_client_core_id
	/* Получаем ключ заемщика */
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS p	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
...
--		, d.client_irbf_nm_up_wgr_kik
	...
	LEFT JOIN [BROADCAST] in_manual_corp_models.risk_segment_replacement_dict	AS rs	/* ETL использует БД dm_corp_models; в прототипе оставляем БД-мануал для пользователей ЛАБ и ТЕСТ */	/* SPMPLN-23739: 1) Добавление справочных значений риск-сегментации */
...
FROM lgd_input AS d
/
COMPUTE STATS sbxc_006_053.lgd_input_vector_tab
/
/* технические проверки прототипа */
/* Дубли по PK */
SELECT report_dt, client_core_id, deal_core_id, COUNT(*)
FROM sbxc_006_053.lgd_input_vector
