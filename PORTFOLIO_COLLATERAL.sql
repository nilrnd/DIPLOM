...
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.portfolio_collateral_tab - техническая таблица с типом хранения данных SCD-2.
	2) dm_corp_models.portfolio_collateral – итоговая пользовательская вью.
--------------

*/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_collateral_tab PURGE
/
CREATE TABLE IF NOT EXISTS sbxc_006_053.portfolio_collateral_tab
(
...
STORED AS PARQUET
/
DROP VIEW IF EXISTS sbxc_006_053.portfolio_collateral
/
CREATE VIEW sbxc_006_053.portfolio_collateral
(
...
AS SELECT 
	...
	, t_changed_dttm
	, t_process_task_id			
FROM sbxc_006_053.portfolio_collateral_tab
WHERE T_DELETED_FLG = 0
	AND	t_active_flg = 1
/
INSERT INTO sbxc_006_053.portfolio_collateral_tab
(
	  report_dt
	, pledger_id
	, coll_core_main_id
...
, collateral AS (
	...
	LEFT JOIN sbxc_006_053.PORTFOLIO_DEAL_REST AS dr
...
/
COMPUTE STATS sbxc_006_053.portfolio_collateral_tab
/
/* технические проверки прототипа */
/* Дубли по PK*/
SELECT report_dt, coll_core_id, COUNT(*)
FROM sbxc_006_053.portfolio_collateral
