...
/*
SPMPLN-14534: Cоздание нового прототипа portfolio_client_asvr_grp (обновленная версия от 04.09.2023). 
...
Прототип на выходе формирует витрины:
	1) data_dm_corp_models.portfolio_client_asvr_grp_tab - техническая таблица.
	2) dm_corp_models.portfolio_client_asvr_grp – итоговая пользовательская вью.
---------------
SPMPLN-15206:
1) переименование атрибутивного состава
2) добавление атрибутов client_osnov_srva_td_with_grp, client_gross_prof_marg_with_grp
3) атрибут client_lrg_ind_nm_bucket стал составным
---------------
SPMPLN-18596: добавление новых атрибутов
---------------
SPMPLN-20181: Проставление комментариев к атрибутам пользовательской вью
*/
DROP TABLE IF EXISTS sbxc_006_053.portfolio_client_asvr_grp_tab PURGE
/
...
WITH grp AS (
	...
	JOIN sbxc_006_053.portfolio_client_asvr AS c	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
		ON c.report_dt = p.report_dt
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS g	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
		ON	g.report_dt = c.report_dt
		AND g.client_stage_id = c.grp_stage_id
		AND c.client_grp_lnk = 'reg_grp'
	LEFT JOIN sbxc_006_053.portfolio_client_asvr AS tg	/* На ПРОДе использовать data_dm_corp_models.portfolio_client_asvr_tab с дополнительным фильтром по t_deleted_flg */
...
/
COMPUTE STATS sbxc_006_053.portfolio_client_asvr_grp_tab
/
